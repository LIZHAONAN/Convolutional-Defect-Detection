...
parser.add_argument("--det_weights", type=str, default=None, help="path to pre-trained detection model")
...
model = TrackingConditionedModel(num_class=2, window_size=64)
if opt.det_weights is not None:
    model.load_from_weights(opt.det_weights)
...
dataset = RecurrentCSVDataset(opt.trainset, method='tracking', non_pos_ratio=opt.non_pos_ratio)
...
weight = torch.FloatTensor([1/5 * opt.non_pos_ratio, 4/5 * opt.non_pos_ratio, 1])
...
if __name__ == '__main__':
    for epoch in range(opt.epochs):
        ...
        path = 'models/tracking_pretrain_{}.pth'.format(epoch)
